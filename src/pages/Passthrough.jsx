...
const Passthrough = () => {
    ...
    const obtenerEnlace = async (code) => {
        ...
        if (response.status === 403) {
            ...
            if (serverReasons.includes("dateActivation") || serverReasons.includes("dateExpiration") || serverReasons.includes("invalid_password")) {
                Swal.fire({
                ...
                Swal.fire({
                    title: "Debes iniciar sesión", icon: "error", text: serverReasons.join(", "),
                }).then(() => {
                    navigate("/login");
                });
            } else {
                if (serverReasons.includes("password")) {
                    setReasons((prevReasons) => ({
                        ...prevReasons, password: true,
                    }));
                }
                if (serverReasons.includes("recaptcha")) {
                    setReasons((prevReasons) => ({
                        ...prevReasons, recaptcha: true,
                    }));
                }
            ...
            Swal.fire({
                title: "Enlace no encontrado", icon: "error", customClass: {
                    popup: "swal__popup",
                    title: "swal__title",
                    icon: "swal__icon",
                    confirmButton: "swal__confirm-button",
            ...
            if (verifyLink(response.data.url)) {
                window.location.href = response.data.url;
            } else {
                Swal.fire({
                    title: "Enlace de redirección inválido", icon: "error", customClass: {
                        popup: "swal__popup",
                        title: "swal__title",
                        icon: "swal__icon",
                        confirmButton: "swal__confirm-button",
        ...
        try {
            ...
            if (reasons.password && !reasons.recaptcha) {
                ...
                if (response.status === 200) {
                    ...
                    Swal.fire({
                        title: "Contraseña incorrecta", icon: "error", customClass: {
                            popup: "swal__popup",
                            title: "swal__title",
                            icon: "swal__icon",
                            confirmButton: "swal__confirm-button",
                ...
                if (response.status === 200) {
                    ...
                    Swal.fire({
                        title: "Recaptcha incorrecto", icon: "error", customClass: {
                            popup: "swal__popup",
                            title: "swal__title",
                            icon: "swal__icon",
                            confirmButton: "swal__confirm-button",
                ...
                if (response.status === 200) {
                    ...
                    Swal.fire({
                        title: "Recaptcha o contraseña incorrectos", icon: "error", customClass: {
                            popup: "swal__popup",
                            title: "swal__title",
                            icon: "swal__icon",
                            confirmButton: "swal__confirm-button",
        ...
        <main className='passth'>
            ...
            <form className='passth__form' onSubmit={handleSubmit} aria-labelledby='passth-title'>
                <fieldset className='form__fieldset'>
                    ...
                    {reasons.password && (<label className='fieldset__label'>Contraseña
                        <input
                            type='password'
                    ...
                    {reasons.recaptcha && (<ReCAPTCHA
                        ref={recaptchaRef}
                        sitekey={import.meta.env.VITE_SITE_KEY_REPACTCHA}
                    />)}

                    {(reasons.recaptcha || reasons.password) && (<button type='submit' className='signup__submit'>
                        Enviar
                    </button>)}
                </fieldset>
            </form>
